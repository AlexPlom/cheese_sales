...
describe('PIKO storefront', () => {
  it('adds products and waives delivery at EUR 20', async () => {
    const user = userEvent.setup()
    render(<App />)

    await user.click(screen.getByRole('button', { name: 'Добави Обикновено сирене' }))
    expect(screen.getByRole('button', { name: /Кошница, 1 артикул, общо €7/ })).toBeInTheDocument()

    await user.click(screen.getByRole('button', { name: 'Увеличи количество Обикновено сирене' }))
    await user.click(screen.getByRole('button', { name: 'Увеличи количество Обикновено сирене' }))
    await user.click(screen.getByRole('button', { name: 'Увеличи количество Обикновено сирене' }))

    expect(screen.getByRole('button', { name: /Кошница, 4 артикула, общо €20/ })).toBeInTheDocument()
  })
  ...
  it('completes the clearly marked demo checkout', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Добави Crystal сирене' }))
    await user.click(screen.getByRole('button', { name: /Кошница, 1 артикул, общо €12/ }))
    ...
    await user.type(screen.getByLabelText('Адрес в Сливен'), 'ул. Хаджи Димитър 1')
